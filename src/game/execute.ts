...
import {
  DEFAULT_OPEN_ACTION,
  FOCUS_ACTION as OPEN_FOCUS_ACTION,
...
const FOCUS_ACTION = OPEN_FOCUS_ACTION;
...
type SimultaneousMovementActorState = {
  position: HexCoord;
  facing: number;
};
...
const isActionActive = (action: string | null | undefined) => {
  ...
  return !isRefreshActionLabel(label) && label !== DAMAGE_ICON_ACTION.toUpperCase();
};
const isPassiveActiveForSameBeatHit = (
...
const isHammerPassiveActive = (entry: BeatEntry | null | undefined) => {
  return isPassiveActiveForSameBeatHit(entry, HAMMER_CARD_ID);
};
const isVengeancePassiveActive = (entry: BeatEntry | null | undefined) =>
  isPassiveActiveForSameBeatHit(entry, VENGEANCE_CARD_ID);
const isOpenBeatAction = (action: string | null | undefined) => {
  return isOpenBeatActionLabel(action);
};
...
const buildSimultaneousMovementPlan = (
  entry: BeatEntry,
  actorId: string,
  actorState: SimultaneousMovementActorState,
  occupancy: Map<string, string>,
  landTiles: HexCoord[],
): SimultaneousMovementPlan | null => {
  ...
  return {
    actorId,
    tokenType: firstMoveToken.type as 'm' | 'j' | 'c',
...
export const executeBeatsWithInteractions = (
  ...
  for (let index = 0; index < normalizedBeats.length; index += 1) {
    ...
    const resolveArrowHit = (
      ...
      const hasHammerPassive = isHammerPassiveActive(targetEntry);
    ...
    const simultaneousMovementActorIdsByBucket = new Map<string, Set<string>>();
    ...
    entriesByTieBucket.forEach((entriesInBucket) => {
      ...
      const bucketActorIds = new Set<string>();
      const movementPlans = new Map<string, SimultaneousMovementPlan>();
      ...
      while (changed) {
        ...
        movementPlans.forEach((plan, actorId) => {
          if (movementTieBlockedBy.has(actorId)) return;
          const findBlockingOccupant = (coord: HexCoord) => {
          ...
          const blockerId =
            plan.tokenType === 'j'
              ...
              : plan.positions.reduce<string | null>((blockedBy, stepPosition) => {
      ...
      const simultaneousActorIds = new Set<string>();
    ...
    const resolveExistingArrows = () => {
      if (!existingArrowIds.size) return;
    ...
    const processOrderedEntry = (entry: BeatEntry) => {
      ...
      tokens.forEach((token) => {
          ...
          if (targetId && blockedByBlock && attackDamage > 0) {
            ...
            if (targetId && !blocked && !targetInvulnerable) {
              if (targetState) {
                ...
                const hasHammerPassive = isHammerPassiveActive(resolvedTargetEntry);
                ...
                if (isVengeancePassiveActive(resolvedTargetEntry)) {
                  applyAdrenalineDeltaToState(targetState, 2);
                }
                if (isThrow) {
        ...
        if (token.type === 'm' || token.type === 'c') {
          ...
          if (!interruptedByArrow && token.type === 'm' && !sameCoord(finalPosition, moveStartPosition)) {
            if (entry.passiveCardId === BURNING_STRIKE_CARD_ID && isExactActionSymbolToken(token, 'm')) {
              queueDelayedPassiveFireHex(index + 1, moveStartPosition, actorId);
            }
            if (entry.passiveCardId === BOW_SHOT_CARD_ID && (rotationMagnitude === 1 || rotationMagnitude === 2)) {
              spawnArrowToken(moveStartPosition, actionSetFacing, actorId);
            }
          }
        }
