...
export const createTutorialGuide = ({
  ...
  const TUTORIAL_STEPS = [
    ...
    },
    {
      ...
      allow: () => [getHandCardById('backflip'), activeSlot],
      highlightElements: () => [getHandCardById('backflip')],
      when: () => isCardInSlot(activeSlot, 'backflip'),
    },
    {
      kind: 'condition',
      position: 'hand',
    ...
    {
      kind: 'wait',
      when: (resolved) => hasActionStarter(resolved.gameState, state.botUserId, 'vengeance', 'advance'),
    },
    {
      kind: 'info',
      position: 'hand',
