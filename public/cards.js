...
const fitAllText = () => {
  document.querySelectorAll('.action-card-surface-row').forEach((row) => fitTextToRow(row));
};
  ...
  try {
    ...
    requestAnimationFrame(() => {
      ...
      if (document.fonts?.ready) {
        document.fonts.ready.then(() => requestAnimationFrame(fitAllText));
      }
    });
    window.addEventListener('load', () => requestAnimationFrame(fitAllText), { once: true });
  } catch (error) {
    console.error(error);
    if (count) {
